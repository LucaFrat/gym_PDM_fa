...
def main(max_iter=100):
    ...
    for d in range(len(path)-1):
        v[d]=path[d+1][0]-path[d][0]/math.cos(path[d][2])
        st[d]=path[d][2]
    print("Velocity - ",v)
    print("Steer-",st)
    # Draw final path
    ...
    # Define the starting point and target point for the rectangle
    start_x = start[0]
    start_y = start[1]
    target_x = goal[0]
    target_y = goal[1]

    # Create a variable to track the rectangle's current position
    rect_x = start_x
    rect_y = start_y
    # Create a variable to track the animation's progress
    progress = 0
    flag_max=len(path)
    f=0
    ...
    while f<flag_max:
        ...
        pos=path[flag_max-1-f]
        # Draw the rectangle at its current position
        for i in range(len(obstacleList)):
            x=obstacleList[i][0]*50
            y=obstacleList[i][1]*50
            R=25
            pygame.draw.circle(screen, (0,0,0), (50+x, 800-y), R)
        pygame.draw.rect(screen, (255, 0, 0), (50+pos[0]*50, 800-pos[1]*50, 10, 10))
        f=f+1
