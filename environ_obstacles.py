...
def environ(show_animation=False):
    ...
    rects = [mpl.patches.Rectangle(rett_points[0], thick,  h_inline, color=obst_color),
             mpl.patches.Rectangle(rett_points[1], thick,  h_inline, color=obst_color),
             mpl.patches.Rectangle(rett_points[2], thick,  h_inline, color=obst_color),

             mpl.patches.Rectangle(rett_points[3], thick,       h_perimeter-h_open, color =obst_color),
             mpl.patches.Rectangle(rett_points[4], thick,       h_perimeter-h_open, color =obst_color),
             mpl.patches.Rectangle(rett_points[5], h_perimeter, thick,              color =obst_color),
    ...
    for i, orig_circ in enumerate(circle_centers): 
        ...
        if i < 3:
            while length <= h_inline:
                ax.add_patch(mpl.patches.Circle((orig_circ[0],orig_circ[1]+length), radius=radi, fill=fill))
            ...
            while length <= h_perimeter:
                ax.add_patch(mpl.patches.Circle((orig_circ[0]+length,orig_circ[1]), radius=radi, fill=fill))
